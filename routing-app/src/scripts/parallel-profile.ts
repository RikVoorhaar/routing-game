...
async function main() {
    try {
        ...
        console.log(`Profiling with ${sampleAddresses.length} addresses using parallel processing...\n`);
...
async function testConcurrency(sampleAddresses: any[], concurrency: number) {
    ...
    console.log(`  🚄 Speed: ${(sampleAddresses.length / (totalTime / 1000)).toFixed(1)} jobs/second`);
}
