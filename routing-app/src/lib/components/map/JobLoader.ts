...
export class JobLoader {
    ...
    async loadJobsForTiles(tiles: string[], minZoom: number = 12): Promise<JobLoadResult | null> {
        console.log('[JobLoader] loadJobsForTiles called with', tiles.length, 'tiles, minZoom:', minZoom);
        ...
        console.log('[JobLoader] Valid tiles (zoom >= minZoom):', validTiles.length, 'tiles:', validTiles);
        ...
        console.log('[JobLoader] Tiles to load (not already loaded):', tilesToLoad.length, 'tiles:', tilesToLoad);
        ...
        try {
            ...
            console.log('[JobLoader] Loaded jobs for', tileResults.length, 'tiles, total jobs:', this.getAllJobs().length);

            // Return info about what tiles were loaded/unloaded for incremental rendering
            return {
                allJobs: this.getAllJobs(),
                newTiles: tileResults.filter(result => result.jobs.length > 0).map(result => result.tileKey),
                removedTiles: removedTiles
            };
