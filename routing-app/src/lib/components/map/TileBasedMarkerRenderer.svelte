<script lang="ts">
    ...
    // New props for tile-based rendering
    export let onTileJobsLoaded: ((tileKey: string, jobs: Job[]) => void) | null = null;
    export let onTileJobsUnloaded: ((tileKey: string) => void) | null = null;
    ...
    export function renderTileJobs(tileKey: string, jobs: Job[]) {
        console.log('[TileBasedMarkerRenderer] Rendering', jobs.length, 'jobs for tile', tileKey);
        ...
        console.log('[TileBasedMarkerRenderer] Created', markers.length, 'markers for tile', tileKey);
    }

    export function clearTileJobs(tileKey: string) {
        const existingMarkers = jobMarkersByTile.get(tileKey);
        if (existingMarkers) {
            console.log('[TileBasedMarkerRenderer] Clearing', existingMarkers.length, 'markers for tile', tileKey);
    ...
    export function clearAllTileJobs() {
        console.log('[TileBasedMarkerRenderer] Clearing all job markers');
    ...
    function updateEmployeeMarkers() {
        ...
        employees.forEach(employee => {
            ...
            try {
                ...
                log.warn(`[TileBasedMarkerRenderer] Failed to create marker for employee ${employee.name}:`, error);
            }
        });
    ...
    function parseRouteData(routeDataString: string | object): PathPoint[] {
        try {
            if (typeof routeDataString === 'string') {
                ...
                log.warn('[TileBasedMarkerRenderer] Invalid route data format:', typeof routeDataString);
                return [];
            }
        } catch (e) {
            log.error('[TileBasedMarkerRenderer] Failed to parse route data:', e);
            return [];
        }
    }
    ...
    function getEmployeePosition(employee: Employee): { lat: number; lon: number } {
        ...
        if (employee.location) {
            try {
                ...
                log.warn(`[TileBasedMarkerRenderer] Invalid location data for employee ${employee.name}:`, e);
            }
        }
