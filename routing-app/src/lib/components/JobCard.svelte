<script lang="ts">
    ...
    import { selectedActiveJob, selectActiveJob, clearSelectedActiveJob, cachedActiveJobs, cacheActiveJobsForJob } from '$lib/stores/activeJobs';
    import { selectedEmployee } from '$lib/stores/selectedEmployee';
    import { employees, currentGameState } from '$lib/stores/gameData';
    ...
    function updateEligibleEmployees() {
        ...
        try {
            ...
            if (eligibleEmployees.length > 0) {
                const defaultEmployee = eligibleEmployees.find(emp => emp.id === $selectedEmployee) || eligibleEmployees[0];
    ...
    async function loadActiveJobsForJob() {
        ...
        try {
            const response = await fetch(`/api/active-jobs?jobId=${$selectedJob.id}&gameStateId=${$currentGameState.id}`);
            if (response.ok) {
                activeJobsForJob = await response.json();
                cacheActiveJobsForJob($selectedJob.id, activeJobsForJob.map(item => item.activeJob));
            } else {
    ...
    async function handleAcceptJob() {
        ...
        try {
            const response = await fetch('/api/active-jobs', {
                ...
                clearSelectedJob();
                clearSelectedActiveJob();
                // Refresh employee data and job list
