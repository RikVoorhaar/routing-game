...
<div
	class="card transform cursor-pointer overflow-visible border-2 p-4 transition-all duration-200 ease-in-out hover:shadow-md"
	...
	aria-label="Route from {startLocation ? formatAddress(startLocation) : 'Unknown'} to {endLocation
		? formatAddress(endLocation)
		: 'Unknown'}"
>
	<div class="card-body p-4">
		...
		<div class="mb-2 flex items-start justify-between">
			...
			<div class="text-right">
				<div class="text-lg font-bold" class:text-success={!isSelected}>
		...
		<div class="space-y-1 text-sm">
			...
			<div class="grid grid-cols-2 gap-2">
				<div>
					<div class="text-xs font-medium uppercase tracking-wide opacity-70">From</div>
					<div class="text-xs leading-tight">
						{startLocation ? formatAddress(startLocation) : 'Unknown location'}
					</div>
				</div>
				<div>
					<div class="text-xs font-medium uppercase tracking-wide opacity-70">To</div>
					<div class="text-xs leading-tight">
						{endLocation ? formatAddress(endLocation) : 'Unknown location'}
					</div>
				</div>
			</div>
