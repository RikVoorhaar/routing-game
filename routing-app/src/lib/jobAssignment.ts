...
export function employeeCanPerformJob(employee: Employee, job: Job): boolean {
    ...
    const jobCategoryValue = typeof job.jobCategory === 'string' 
        ? parseInt(job.jobCategory, 10) 
        : job.jobCategory;
    const jobCategory = jobCategoryValue as JobCategory;
    
    // Validate that the job category is within valid range
    if (isNaN(jobCategoryValue) || jobCategoryValue < 0 || jobCategoryValue > JobCategory.TOXIC_GOODS) {
