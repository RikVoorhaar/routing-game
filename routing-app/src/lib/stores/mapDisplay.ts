...
export const mapDisplayActions = {
    ...
    removeRoute(routeId: string) {
        displayedRoutes.update(routes => 
            routes.filter(r => r.route.id !== routeId)
        );
    },
    ...
    selectRoute(routeId: string) {
        displayedRoutes.update(routes => 
            routes.map(r => ({
                ...
                style: r.route.id === routeId ? ROUTE_STYLES.SELECTED : 
                       r.isAvailable ? ROUTE_STYLES.AVAILABLE :
                       r.isActive ? ROUTE_STYLES.ACTIVE : ROUTE_STYLES.DEFAULT
            }))
        );
    },
