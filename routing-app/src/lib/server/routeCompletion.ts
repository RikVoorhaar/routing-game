...
export async function processCompletedRoutes(gameStateId: string): Promise<{
    ...
    try {
        ...
        for (const { employee, activeJob, jobRoute, endAddress } of employeesWithActiveJobs) {
            ...
            if (activeJob.currentPhase === 'traveling_to_job' && activeJob.routeToJobId) {
                ...
                if (activeJob.currentPhase === 'traveling_to_job') {
                    ...
                    const jobPhaseStartTime = activeJob.jobPhaseStartTime ? new Date(activeJob.jobPhaseStartTime).getTime() : jobStartTime + travelTime;
            ...
            if (elapsed >= totalDuration) {
                // Job is completed
                const rewardNum = typeof jobRoute.reward === 'string' ? parseFloat(jobRoute.reward) : jobRoute.reward;
        ...
        if (completedJobUpdates.length > 0) {
            ...
            console.log(`Processed ${processedRoutes} completed active jobs for game ${gameStateId}, total reward: ${totalReward}`);
        }
